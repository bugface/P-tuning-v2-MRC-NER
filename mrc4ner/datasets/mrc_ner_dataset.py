...
class MRCNERDatasetPtuning(Dataset):
    def __init__(
        self,
        json_path,
        tokenizer: BertWordPieceTokenizer,
    ...
    def __getitem__(self, item):
        ...
        # query = data["query"]
        query = torch.arange(self.query_len) + (label_idx - 1) * self.query_len
        ...
        if not self.is_chinese:
            for token_idx in range(len(tokens)):
                ...
                next_word_idx = (
                    query_context_tokens.word_ids[token_idx + 1]
                    if token_idx + 1 < len(tokens)
                    else None
                )
                prev_word_idx = (
...
def run_dataset():
    """test dataset"""
    import os
    from collate_functions import collate_to_max_length_ptuning, collate_to_max_length
